...
def load_calibration_data(file_path:str, file_name:str):
    """
    ...
    """
    try:
        data = pd.read_csv(os.path.join(file_path, file_name), delimiter='\t', header=None)
        return np.asarray(data[0]), np.asarray(data[1]//0.0215)
    except Exception as e:
        print(f"Error loading calibration data: {e}")
        return None
    
def load_experimental_data(file_path:str, file_name:str):
    """
...
def calibrate_rmp_signal(p_ref:np.array, p_rmp:np.array, p_target:np.array, time:np.array, nperseg:int=1024):
    ...
    epsilon = 1e-3 * np.max(np.abs(H_interp)) # Regularization term to avoid division by zero
    H_inv = np.conj(H_interp) / (np.abs(H_interp)**2 + epsilon) # Inverse of the transfer function with regularization
    
    p_calibrated_fft = p_target_fft * H_inv
    p_calibrated_t = np.fft.irfft(p_calibrated_fft, n=N)
    
    return f, H, Cxy, delay, group_delay, p_calibrated_t
