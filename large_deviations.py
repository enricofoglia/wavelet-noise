...
def _main():
    ...
    diagnostics = compute_diagnostics(
        signal, dt=case.time[1] - case.time[0], corr_threshold=0.0
    )
    ...
    ax[2].grid()
    fig.legend(
        [emp[0], gaus[0], gaus_emp[0]],
        ["Data", "Gaussian (analytical)", "Gaussian (empirical)"],
        loc="upper center",
    ...
    ax.grid()
    plt.savefig(
        os.path.join(args.fig_dir, "large_deviations_hist.pdf"), dpi=300, bbox_inches="tight"
    )
    ...
    for m in range(n_steps,0,-1):
        b = len(signal) // diagnostics["effective_samples"] * m 
        scgf = empirical_scgf(noise_st, k, b=b)
        s_k, s, I = empirical_rate_func(noise_st, k, b=b)
    ...
    fig.colorbar(sm, cax=cax, ticks=np.arange(1, n_steps + 1), label=r"$\Delta t / \tau_c$")
    plt.savefig(
        os.path.join(args.fig_dir, "large_deviations_b_convergence.pdf"), dpi=300, bbox_inches="tight"
    )
